...
| 1.10 | **Two plan labels were matched to rooms by elimination.** The venue plan has no "Mask Room" or "Admin Office"; the zones assume **BOARD ROOM → maskRoom** and **BACK OFFICE → adminOffice** because those were the unmatched pairs. | Wrong means two rooms' zones sit on the wrong physical spaces, and a beacon plan inherits the error. | Assumed in `shows/MAD-DIM.json`; each is a polygon swap to fix. |
| 1.11 | **`DONE` as a room event.** Not in the contract, sent by nothing, used only by the operator button. Either give it a distinct meaning from `RELEASE` (content finished *with people still there*) or drop it. | Two events that look identical get used interchangeably. | Convention in the demo shows. |
...
| **Zone drawing** | The real venue plan arrived (`public/assets/mad-floorplan.svg` — vector, so it stays crisp when a future tool zooms in) and all 23 zones are traced over it — by eye, as boxes with deliberate gaps, ±15px. Good enough for rehearsal; not good enough to place beacons against. The tracing tool still does not exist and is the right way to do the precise pass. The no-two-rooms-claim-one-point invariant is now a load-time warning in the validator, so hand-nudged vertices cannot silently break it. Zone coordinates live in the floorplan box (784×1510, origin top-left); polygons take any number of points ≥3, concave included, and a room may own several zones. |
