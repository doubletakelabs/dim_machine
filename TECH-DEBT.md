...
| **Zone drawing** | The real venue plan arrived (`public/assets/mad-floorplan.png`, 784×1510) and all 23 zones are now traced over it — but by eye, as axis-aligned boxes with deliberate gaps, accurate to maybe ±15px. Good enough for rehearsal and for the panel to mean something; not good enough to place beacons against. The tracing tool still does not exist, and is the right way to do the precise pass. An audit script confirmed no point on the plan classifies into two rooms — keep that invariant when redrawing. |
