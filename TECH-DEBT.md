...
| ~~1.2~~ | ~~Kinds for the post-museum rooms.~~ | **Resolved** (2026-09-13): only the DIM rooms can be "theirs" — everything else is open to anyone. All five post-museum rooms are `shared`: no lock, no capacity, run for the space, everyone present. | Done. |
...
| **The server is http only** | Which costs more than it looks. `navigator.wakeLock`, and every other API gated on a secure context, is simply undefined on the `http://192.168.x.x` a phone uses on venue wifi — so the screen-sleep fix falls back to a muted looping video. Measured on the Samsung A15 over real wifi (2026-09-14, `/handset-check.html`): the fallback video **does** hold its screen awake (verified against a shortened screen timeout), but **`devicemotion` never fires on an insecure origin — the shake gesture is dead over http**, full stop, not degraded. Audio and websockets are unaffected: 8ms RTT, audio at 100% through 18s of screen-off. Self-signed https means trusting a profile on every handset; a real cert means a domain resolving on a network with no internet. Worth deciding before load-in rather than at it. |
